...
void _td_io_detection(IOReadWrite rw, FILE* file, Detection* detection);
inline void _td_io_detection_bo(size_t idxconfig, size_t idxconfigstart, Reducer* ths, MANY(MinMaxIdx) th_indexes, MANY(Detection)* detections, RWindow window);
...
typedef struct windowdetection_context {
	tdqueue_data** qm;
	tdqueue_t queue;
	pthread_t producer;
...
void* _windowdetection_producer(void* argsvoid) {
    ...
    for (size_t i = 0; i < many.number; i++) {
        ...
        _td_io_path(args->outdir, path, split);
...
void* _windowdetection_consumer(void* argsvoid) {
    ...
    while(!tdqueue_isend(args->queue)) {
        ...
        // CLOCK_START(_windowdetection_consumer);
        ...
        for (size_t idxconfigstart = 0; idxconfigstart < n_configs; idxconfigstart += blockconfig_step) {
            ...
            for (size_t w = 0; w < split->test->all->number; w++) {
                ...
                FOR_CONFIG {
                    ...
                    _td_io_detection(IO_WRITE, file, &detection_training_best);
                    _td_io_detection(IO_WRITE, file, &detection_test);
                }
        ...
        free(qm);
        // CLOCK_END(_windowdetection_consumer);
    }
...
void* windowdetection_start() {
    char outdir[PATH_MAX];

    windowdetection_context* context = calloc(1, sizeof(windowdetection_context));
    ...
    pthread_create(&context->producer, NULL, _windowdetection_producer, &context->producer_args);
    ...
    for (size_t i = 0; i < TD_NTHREADS; ++ i) {
        ...
        pthread_create(&context->consumers[i], NULL, _windowdetection_consumer, &context->consumer_args[i]);
    }

    return context;
}

void windowdetection_wait(void* context_void) {
    assert(context_void);

    windowdetection_context* context = context_void;
...
inline void _td_io_detection_bo(size_t idxconfig, size_t idxconfigstart, Reducer* ths, MANY(MinMaxIdx) th_indexes, MANY(Detection)* detections, RWindow window) {
    size_t apply_idxconfig = idxconfig + idxconfigstart;
    size_t idxth_start = th_indexes._[idxconfig].min;
    size_t idxth_end = th_indexes._[idxconfig].max;
    WApply * apply = &window->applies._[apply_idxconfig];
...
inline void _td_io_path(char outdir[PATH_MAX], char path[PATH_MAX], RWindowSplit split) {
