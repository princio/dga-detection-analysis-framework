...
for nn in [7, 8, 9, 10]:
    ...
    for _, pcap in pcaps_good.iterrows():
        ...
        fp_tot_uniques += (df.drop_duplicates(subset=['dn_id'])['value'] > 0.5).sum()
    ...
    print(nn)
    print(f"{fp_tot:10d}\t{tot:10d}\t{fp_tot/tot}")
    print(f"{fp_tot_uniques:10d}\t{tot_uniques:10d}\t{fp_tot_uniques/tot_uniques}")
