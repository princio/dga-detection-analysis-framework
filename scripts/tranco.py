...
def main(
    ...
    for i in range(nbatch):
        if Path(f'./lstm/tranco/tranco_{i}.csv').exists():
            continue

        print(f'{i + 1}/{nbatch + 1}')
        
        df = pd.read_csv('/Users/princio/Repo/princio/malware-detection-predict-file/tranco/tranco_1m.csv', skiprows=batch * i, nrows=batch)
        df.columns = [ 'id', 'whitelist_id', 'dn', 'rank' ]
        ...
        df[[ 'id', 'whitelist_id', 'dn', 'rank', 'eps_none', 'eps_tld', 'eps_icann', 'eps_private', 'tld', 'icann', 'private']]\
            .to_csv(f'./lstm/tranco/tranco_{i}.csv', index=False)
