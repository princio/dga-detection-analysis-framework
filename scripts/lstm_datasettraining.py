...
def main(
    ...
    for i in range(nbatch):
        if Path(f'./lstm/training/training_{i}.csv').exists():
            continue

        print(f'{i + 1}/{nbatch + 1}')
        
        df = pd.read_csv('/Users/princio/Repo/princio/malware-detection-predict-file/asset/ml/dataset_training.csv', index_col=None, skiprows=batch * i, nrows=batch)
        print(df)
        ...
        df[[ 'legit', 'class', 'dn', 'eps_none', 'eps_tld', 'eps_icann', 'eps_private', 'tld', 'icann', 'private']]\
            .to_csv(f'./lstm/training/training_{i}.csv', index=False)
