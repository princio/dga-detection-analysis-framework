...
def qt_pcaps(qt: QT, pcaps: List[Tuple[str, int]], fetch_config: FetchConfig, step = 8, renews = [False], th_s: List[float]=[1.]):
    dfs = []
    ...
    aggs = {}
    for aa in ['A', 'ADay']:
        aggs[QTRowCM._(aa, 'gr')] = 'sum'
        aggs[QTRowCM._(aa, 'gnx')] = 'sum'
        aggs[QTRowCM._(aa, 'gok')] = 'sum'
        aggs[QTRowCM._(aa, FetchFieldMetric.g_pos('r'))] = 'sum'
        aggs[QTRowCM._(aa, FetchFieldMetric.g_pos('ok'))] = 'sum'
        aggs[QTRowCM._(aa, FetchFieldMetric.g_pos('nx'))] = 'sum'
    ...
    #     tot = df_overall[(QTRowCM._('TA', e), 'sum')] + df_overall[(QTRowCM._('MA', e), 'sum')]
    #     for a in ['TA', 'MA']:
    #         df_overall[(QTRowCM._(a+'R', e), 'ratio')] = df_overall[(QTRowCM._(a, e), 'sum')] / (tot)
    #     df_overall[(QTRowCM._(f'TDR', e), 'ratio')] = df_overall[(QTRowCM._('PDay', e), 'sum')] / (df_overall[(QTRowCM._('ADay', e), 'sum')] + df_overall[(QTRowCM._('PDay', e), 'sum')])
