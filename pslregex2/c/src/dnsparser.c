...
int main(int argc, char **argv) {
    ...
    // printTldTree(conf.tld_tree, "ciao_");

    fprintf(conf.file, "time,size,protocol,server,message_ID,qr,AA,rcode,dn,qcode,qdcount,ancount,nscount,arcount,answer\n");
    ...
    ****** MODIFIED BY OVERET */
    ...
    while (qnext != NULL) {
        ...
        if (conf->PRINT_RR_NAME) {
            ...
            if (parser->name == NULL) 
                printf("%s UNKNOWN(NULL,%d)", qnext->name, qnext->type);
            else 
...
void print_summary2(ip_info * ip, transport_info * trns, dns_info * dns,
    ...
    sprintf(ts, "%d.%06d", (int)header->ts.tv_sec, (int)header->ts.tv_usec);
    fprintf(conf->file, "%s,%d,%c,%s,%u,%c,%s,%hhu,%ld,",
        ts,
    ...
    if (0) { // BDN
        char *tld;
        if(tld_get_z(first_name, &tld) == TLD_SUCCESS)
            fprintf(conf->file, "%s\n", tld);
        else
            fprintf(conf->file, "\n");
    }
